...
var require_openapi = __commonJS({
  "openapi/openapi.json"(exports, module) {
    module.exports = {
      openapi: "3.0.3",
      info: {
      ...
      paths: {
        ...
        "/openapi.json": {
          get: {
            summary: "Descargar OpenAPI JSON",
            responses: {
              "200": {
                description: "OK"
              }
...
var appointmentRequestSchema = external_exports.object({
  insuredId: external_exports.string().regex(/^\d{5}$/, "insuredId debe ser 5 d\xEDgitos"),
  scheduleId: external_exports.number().int().positive(),
  countryISO: external_exports.enum(["PE", "CL"])
});

// src/infrastructure/persistence/DynamoRepository.ts
import { DynamoDBClient } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, PutCommand, QueryCommand, UpdateCommand } from "@aws-sdk/lib-dynamodb";
var client = new DynamoDBClient({});
var doc = DynamoDBDocumentClient.from(client);
...
var DynamoRepository = class {
  ...
  async markCompleted(appointmentId) {
    ...
    await doc.send(new UpdateCommand({
      ...
      ExpressionAttributeValues: { ":s": "completed", ":u": (/* @__PURE__ */ new Date()).toISOString() }
    }));
  }
};

// src/infrastructure/messaging/Messaging.ts
import { SNSClient, PublishCommand } from "@aws-sdk/client-sns";
import { EventBridgeClient, PutEventsCommand } from "@aws-sdk/client-eventbridge";
var sns = new SNSClient({ region: process.env.AWS_REGION });
var eb = new EventBridgeClient({ region: process.env.AWS_REGION });
var SNS_TOPIC_ARN = process.env.SNS_TOPIC_ARN;
var EVENT_BUS_NAME = process.env.EVENT_BUS_NAME || "appointment-bus";
async function publishToSns(payload, countryISO) {
  if (!countryISO || !["PE", "CL"].includes(countryISO)) {
    console.warn("\u26A0\uFE0F countryISO inv\xE1lido o no definido, no se publicar\xE1 en SNS", countryISO);
    return;
  }
  console.log("\u{1F4E4} Publicando en SNS", { TopicArn: SNS_TOPIC_ARN, countryISO, payload });
  try {
...
// src/infrastructure/api/appointment.ts
var ddb = new DynamoRepository();
async function handler(event) {
  ...
  if (event.Records && Array.isArray(event.Records)) {
    for (const rec of event.Records) {
      ...
      if (appointmentId) {
        console.log(`\u{1F504} Marcando cita completada: ${appointmentId}`);
...
async function createAppointment(event) {
  try {
    ...
    if (!parsed.countryISO || !["PE", "CL"].includes(parsed.countryISO)) {
      return { statusCode: 400, body: JSON.stringify({ error: "countryISO inv\xE1lido o no proporcionado" }) };
    }
    ...
    console.log("\u{1F4E4} Publicando en SNS con countryISO:", parsed.countryISO);
...
async function openapi(_event) {
  const spec = (await Promise.resolve().then(() => __toESM(require_openapi(), 1))).default;
